...
fn generate_c_header(crate_dir: &str) {
    ...
    let version_block = format!(
        "#define RADSEX_VERSION \"{version}\"\n\
         #define RADSEX_VERSION_MAJOR {major}\n\
         #define RADSEX_VERSION_MINOR {minor}\n\
         #define RADSEX_VERSION_PATCH {patch}",
        version = version,
        major = parts[0],
        minor = parts[1],
        patch = parts[2],
    ...
    cbindgen::Builder::new()
        ...
        .write_to_file(output_dir.join("radsex.h"));

    let path = output_dir.join("radsex.h");
