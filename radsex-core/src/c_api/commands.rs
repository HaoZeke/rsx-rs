...
unsafe fn cstr_to_string(ptr: *const c_char, name: &str) -> Result<String, radsex_status_t> {
    if ptr.is_null() {
        set_last_error(&format!("null pointer for {name}"));
        return Err(radsex_status_t::RADSEX_INVALID_PARAMETER);
    }
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn radsex_process(
    ...
    catch_unwind(|| {
        ...
        match crate::commands::process::run(&params) {
            Ok(()) => radsex_status_t::RADSEX_SUCCESS,
            Err(e) => {
...
/// All string pointers must be valid null-terminated C strings.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn radsex_freq(
