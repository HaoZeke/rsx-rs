...
The tracked paper-facing panel contains four completed datasets
...
|--------------------------+---------+--------------+------------+-------------------|
| =danio_albolineatus=     |      58 | 10,200,185,246 | 29,357,812 |         1,240.187 |
| =notothenia_rossii=      |      42 | 10,468,745,314 | 11,337,736 |         1,104.048 |
| =plecoglossus_altivelis= |      65 |  9,924,715,896 |  7,338,124 |         1,045.383 |
| =tinca_tinca=            |      43 | 11,257,335,754 |  6,932,387 |         1,020.880 |
...
Generated assets live in =docs/figures/= (tracked SVGs + TSVs). Key plots are
embedded below; regenerate with =pixi run -e benchmark plot-literature-benchmarks=.

#+begin_export rst
.. figure:: /_static/figures/literature_radsex_speedups.svg
   :alt: Geometric-mean speedups of rsx-rs vs C++ RADSex across literature datasets
   :width: 100%
   :align: center

   Literature speed comparison (rsx-rs vs C++ RADSex). Source: ``docs/figures/literature_radsex_speedups.svg``.

.. figure:: /_static/figures/literature_runtime_breakdown.svg
   :alt: Runtime breakdown across process and downstream stages
   :width: 100%
   :align: center

   Runtime breakdown. Source: ``docs/figures/literature_runtime_breakdown.svg``.

.. figure:: /_static/figures/literature_compute_breakdown.svg
   :alt: Compute time breakdown
   :width: 100%
   :align: center

   Compute breakdown. Source: ``docs/figures/literature_compute_breakdown.svg``.
#+end_export

TSV companions (not embedded): =literature_benchmark_summary.tsv=,
=literature_speedup_summary.tsv=, =literature_speedup_pairs.tsv=. Full SVG list
on disk: =literature_*_*.svg= under =docs/figures/=.
...
reproducibility archive (doi:10.5281/zenodo.20531539) regenerates every
...
tracked in =benchmarks/results/literature_bayesian_evidence.csv=.

#+begin_export rst
.. figure:: /_static/figures/literature_bayesian_evidence.svg
   :alt: Bayesian evidence summary across literature datasets
   :width: 100%
   :align: center

   Bayesian biological evidence. Source: ``docs/figures/literature_bayesian_evidence.svg``.

.. figure:: /_static/figures/literature_candidate_recovery.svg
   :alt: Candidate recovery strict FASTA vs posterior-supported markers
   :width: 100%
   :align: center

   Candidate recovery (strict Bonferroni FASTA vs posterior-supported markers).
#+end_export
...
counts go to =benchmarks/results/prior_sensitivity_from_triage.csv=.

#+begin_export rst
.. figure:: /_static/figures/literature_prior_sensitivity_heatmap.svg
   :alt: Prior sensitivity heatmap for triage parameters pi and p_sex
   :width: 100%
   :align: center

   Prior sensitivity heatmap (``pi`` × ``p_sex``). Large asset; scroll if needed.
#+end_export
...
|--------------+----------------------------|
| RADSex output compatibility | compare directly with prior RADSex biological results |
...
| Numerical/statistical routines | =rsxcore/tests/test_precision.rs= and =rsxcore/src/stats.rs= unit tests |
| Command behavior | =rsxcore/tests/test_pipeline.rs= integration tests |
...
| =MarkerTableSource= parity (file / Arrow / Parquet) | =rsxcore/src/source/tests.rs= and =rsx-python/tests/test_marker_table_from_arrow.py= (including the =RSX_FORCE_SPILL=1= toggle test) |
...
=rsxcore/src/source/= rather than materialising a markers TSV behind
the user's back.

The implementation has three variants:
...
=rsxcore/src/source/tests.rs= asserts this for =freq= on a shared
...
=rsxcore/src/source/estimator.rs= and combines four factors:
...
with a single =libm::erfc= call (108 ns). The CUDA kernels call the device
=erfc= for the same reason.
