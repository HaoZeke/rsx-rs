...
** Why is the binary called =rsx= but the crate =rsxcore=?

The command-line tool is =rsx= (crate =rsx-cli=); the library crate is
=rsxcore= on crates.io. The Python package is =pyrsx= on PyPI. The
...
** How do I install the R bindings?

The package is =rsxr= in the =rsx-r/= subdirectory of this repository (not yet
on CRAN). With =cargo= on =PATH=:

#+begin_src r
pak::pak("HaoZeke/rsx-rs/rsx-r")
#+end_src

See the Language bindings page and =rsx-r/README.md=.

#+begin_export rst
Cross-link to the Language bindings page.
#+end_export

CI runs R-CMD-check and rOpenSci =pkgcheck=.
...
** How do I get sex-linked marker probabilities in addition to calls?
...
#+begin_export rst
#+begin_export rst
See the :doc:`/used_by` page and the :doc:`/explanation/benchmarks` page.
#+end_export
#+end_export
