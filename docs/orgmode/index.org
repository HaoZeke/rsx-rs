#+TITLE: rsx-rs Documentation
#+AUTHOR: Rohit Goswami, Ruhila Goswami
#+OPTIONS: toc:2 num:t

* rsx-rs

#+begin_export rst
.. image:: /_static/logo-light.svg
   :class: light-only
   :width: 460
   :align: center
   :alt: rsx

.. image:: /_static/logo-dark.svg
   :class: dark-only
   :width: 460
   :align: center
   :alt: rsx
#+end_export

~rsx~ is a Rust framework for RAD-seq marker analysis and sex determination. It
pairs bounded-memory streaming kernels with a Bayesian marker-evidence layer and
Python and C bindings. It builds on and stays command-compatible with
[[https://github.com/RomainFeron/RADSex][RADSex]], so existing analyses transfer
unchanged and prior biological results remain directly comparable.

** Features

- Original RADSex-compatible commands: =process=, =distrib=, =signif=,
  =depth=, =freq=, =map=, =subset=
- Sex-linked marker evidence command: strict calls, Bayes factors, posterior
  P(sex-linked), penetrance, bias direction, and marker class
- =merge= command: external sort-merge for 75M+ sequence tables
- =pca= command: streaming sample PCA in O(n_individuals^2) memory
- Bayesian marker evidence: Bayes factors and symmetric posterior
  probabilities for sex-linked enrichment in either compared group
- Byte-identical output to C++ RADSex (when groups specified explicitly)
- Bounded-memory streaming: all commands work on 50GB+ tables
- Parallel file processing via [[https://docs.rs/rayon][rayon]]
- minimap2 alignment (replaces BWA-MEM)
- Optional Parquet output for merge (=--features parquet-io=)
- Language bindings: Python (=pyrsx= on PyPI), R (=rsxr= in =rsx-r/=), C API
  (cbindgen); see
#+begin_export rst
:doc:`Language bindings </reference/bindings>`
#+end_export
  and the Rust crate API
#+begin_export rst
:doc:`Rust API Reference </reference/rust-api>`
#+end_export
- Cross-platform: Linux, macOS (Intel + Apple Silicon), Windows (without map)
- SymPy/Sollya mathematical proofs for all optimizations

** Quick start

#+begin_src bash
# Install from source
git clone https://github.com/HaoZeke/rsx-rs.git
cd rsx-rs
cargo build --release
cp target/release/rsx ~/.local/bin/

# Or via pixi
pixi run build

# Python bindings (high-level API for notebooks/workflows)
pip install pyrsx
# or pixi run -e python build-python

# R bindings (in-process; needs cargo — see rsx-r/README.md)
# pak::pak("HaoZeke/rsx-rs/rsx-r")

# Run the pipeline
rsx process -i reads/ -o markers.tsv -T 8
rsx distrib -t markers.tsv -p popmap.tsv -o distrib.tsv -G M,F
rsx signif -t markers.tsv -p popmap.tsv -o signif.tsv -G M,F
rsx map -t markers.tsv -p popmap.tsv -g genome.fa -o aligned.tsv -G M,F
#+end_src

#+begin_export rst
.. toctree::
   :caption: Getting started
   :maxdepth: 1

   tutorials/quickstart

.. toctree::
   :caption: Reference
   :maxdepth: 2

   reference/rust-api
   reference/c-api
   reference/commands
   reference/glossary

.. toctree::
   :caption: How-to guides
   :maxdepth: 1

   howto/r-integration
   howto/faq
   howto/troubleshooting

.. toctree::
   :caption: Explanations & meta
   :maxdepth: 1

   explanation/architecture
   explanation/benchmarks
   explanation/citation
   changelog
   code_of_conduct
   used_by
#+end_export
