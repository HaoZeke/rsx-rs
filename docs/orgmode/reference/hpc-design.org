...
At production scale (5M markers x 200 individuals, 5GB raw data), the
bottleneck stack is:
...
|-----------------------------+---------------+------------|
| Baseline implementation     | 1x            | 1.8x vs C++ |
...
| GPU offload                 | I/O saturation only | workload-dependent |
...
Bonferroni correction requires knowing n_markers before output.
Two passes over the mmap'd table:
1. Pass 1: count n_markers (fast, no sequence parsing)
2. Pass 2: compute p-values with known threshold, write directly

*** External sort-merge (merge, depth streaming)

For datasets exceeding RAM: chunked sort with lz4 temp files + k-way
merge via BinaryHeap. The depth command sorts only non-zero entries
(sparse optimization, 3.3x I/O reduction for 70% sparse data).

Proof: =scripts/sympy/sparse_median_proof.py=

*** Streaming sample PCA via Gram eigendecomposition (pca)

For an (m x n) depth matrix where m >> n (75M markers, 200 individuals):
- Stream rows, accumulate X^T X (n x n = 200 x 200 = 320KB)
- Eigendecompose X^T X (Jacobi rotation, instant for 200x200)
- Eigenvectors are the sample loading directions from a full PCA of X

This is mathematically exact (not an approximation):
  SVD: X = U S V^T
  Gram: X^T X = V S^2 V^T
  Therefore: eigvecs(X^T X) = V, the sample-side PCA directions
...
- Mixed-precision (f32 chi-squared, f64 CDF): =radsex-core/tests/test_precision.rs=
...
- Streaming Gram eigendecomposition equals full-matrix sample PCA:
  =scripts/sympy/tucker_covariance_proof.py=
