...
strict positive controls, strict null calls, and posterior-ranked candidate
triage under the same published workflow.
...
dataset, so the Bayesian summary adds a biological triage view that is not
available from strict thresholded FASTA output alone.

The candidate-recovery figure compares strict Bonferroni FASTA marker counts
with posterior-ranked candidates at the same dataset/depth settings. This
...
ranked follow-up set when all-or-nothing FASTA extraction is empty.
...
same marker table. The Bayesian table gives a ranked candidate layer: it
recovers 49 high-posterior candidates for =plecoglossus_altivelis=, expands the
=tinca_tinca= follow-up set to 11, and distinguishes the two strict nulls
...
|---------+--------------------+-------------+----------------+-------------------+----------------|
| =danio_albolineatus= | 0 | 0 | 30 | 158 | posterior triage without strict reclassification |
| =notothenia_rossii= | 0 | 0 | 0 | 400 | QC restraint on Bayes-factor-only signal |
| =plecoglossus_altivelis= | 47 | 0 | 2 | 523 | strict recovery plus posterior expansion |
| =tinca_tinca= | 6 | 0 | 5 | 432 | strict recovery plus posterior expansion |
...
posterior ranking recovers every strict d=10 marker and adds two
=plecoglossus_altivelis= and five =tinca_tinca= follow-up markers. In
=danio_albolineatus=, rsx exposes 30 high-posterior exploratory candidates
while preserving the strict null call. In =notothenia_rossii=, 400 rows have
...
| =tinca_tinca= | confirmatory | XX/XY-supported | 6 male-biased strict markers; 11 high-posterior markers for secondary triage |
| =danio_albolineatus= | exploratory | ZZ/ZW-like validation set | 30 high-posterior markers without strict reclassification; 29 are female-biased |
| =notothenia_rossii= | restrained null | no high-posterior sex-system call | 400 Bayes-factor-only rows, 0 high-posterior rows, 52.78% singleton retained markers |
...
| Exploratory ranking | FDR, Bayes posterior | prioritize candidates below strict Bonferroni calls |
...
| Additional statistical modes | compare strict tests, FDR, and posterior candidate ranking on the same counts |
...
| Bayesian biological triage | =benchmarks/results/literature_bayesian_evidence.csv= and candidate-recovery figures |
