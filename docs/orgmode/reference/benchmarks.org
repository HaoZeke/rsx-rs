...
Comparison of radsex-rs (Rust) vs the original RADSex (C++11) across three
...
radsex-rs originally copied this pattern using crossbeam channels, but
...
=std::unordered_map<string, uint>= with the default hash. radsex-rs uses
...
a manual thread pool with mutex-protected work-stealing. radsex-rs uses
...
radsex-rs uses minimap2 (via Rust bindings) instead of BWA-MEM. minimap2 is
...
# Build Rust radsex-rs
cd /path/to/radsex-rs && cargo build --release
...
Despite the performance differences, radsex-rs produces *byte-identical
