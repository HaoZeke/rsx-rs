...
The tracked numbers go to =benchmarks/results/marker_source_paths.csv=.
Wall-time and peak-RSS columns let the paper report the cost of the
DataFrame API relative to the streaming file path; see
=benchmarks/bench_marker_source.py= for the full instrumentation. The
specific build-host numbers are tracked in the CSV; the qualitative claim
is that the in-memory Arrow path is constant-factor heavier than the
file streamer (one extra IPC serialisation step at construction, the
per-marker work is identical), and the Parquet spill path adds a single
spill-then-stream pass whose cost the Beissinger estimator only accepts
when the alternative is exceeding the RAM budget.
