...
| =--correction= | bonferroni | Multiple testing (see note below) |
| =--test= | chisq | Statistical test: chisq, fisher, gtest |
| =--bayes= | | Add Bayes Factor + posterior columns per cell |
| =-C= | false   | Legacy alias for =--correction none= |

Correction note:

- =bonferroni= follows RADSex: each cell's p is compared to =S / n_markers=
  (marker-table size), preserving byte-compatible significance flags with
  C++ RADSex when groups are explicit.
- =fdr= applies Benjamini–Hochberg across markers. The implementation computes
  one p-value per presence-count cell and weights it by the cell's =Markers=
  count, which is equivalent to applying the procedure to the expanded list of
  marker p-values without materializing that list.
- =none= uses the raw threshold =S= on each cell p-value.
...
| =--test=       | chisq      | Statistical test: chisq, fisher, gtest           |
| =--backend=    | cpu        | P-value backend: cpu or cuda                     |
| =--bayes=      |            | Add Bayes Factor + posterior P(sex-linked) cols  |
| =-a=           |            | Output FASTA instead of table                    |

Two-pass streaming for Bonferroni / none: pass 1 counts markers, pass 2 writes
directly with O(n_individuals) working memory.

FDR correction (=--correction fdr=) stores all marker p-values (O(n_markers)
...
per-individual columns are the same right singular vectors (sample-space
axes / scores) that full-matrix PCA would produce; the marker table does
not need to be materialized. These are *not* classical marker feature loadings.
...
- =loadings.tsv=: individual × component sample scores (legacy filename)
- =sample_scores.tsv=: same matrix under the clearer name
