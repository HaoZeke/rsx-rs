...
Correction note (not interchangeable):
...
- =fdr= applies Benjamini–Hochberg across the *cell grid*
  (=(n_group1+1)×(n_group2+1)−1= non-empty presence-count cells), not across
  markers. =CorrectedP= / =Signif= under FDR are therefore *not* a milder
  version of the Bonferroni column — different multiple-testing universe.
