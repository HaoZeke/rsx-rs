...
The output joins RADSex-style strict evidence and Bayesian evidence in one
marker-level table. Columns include group penetrance, bias direction,
p-value, Bonferroni-corrected p-value, Bayes factor, posterior
P(sex-linked), and =Candidate_Class=. Classes are:
=strict+posterior=, =strict_only=, =posterior_only=, and
=bayes_factor_only=.

This command is the core biological triage endpoint. Strict calls remain
confirmatory; posterior-only rows are validation targets; Bayes-factor-only
rows are warnings that should be interpreted with depth, frequency, and PCA
QC outputs.
