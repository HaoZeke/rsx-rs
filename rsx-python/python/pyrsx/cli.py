...
@main.command()
@click.option(
    "-i", "--input-dir", required=True, help="Directory of FASTQ/FASTA files."
)
...
@click.option(
    ...
    default=1,
    show_default=True,
    help="Min depth to retain marker.",
)
@click.option(
    "-k", "--kmer-dedup", default=None, type=int, help="K-mer size for deduplication."
)
...
@main.command()
@click.option(
    "-t", "--markers-table", required=True, help="Marker depth table from process."
)
@click.option(
    "-p", "--popmap", required=True, help="Population map (individual<tab>group)."
)
